...
<?php if(empty($taxonomies)) return; ?>
...
if (!function_exists('woof_print_tax'))
{
    ...
    function woof_print_tax($taxonomies, $tax_slug, $terms, $exclude_tax_key, $taxonomies_info, $additional_taxes, $woof_settings, $args, $counter)
    {
        ...
        elseif( ( count( $taxonomies ) % 3 ) == 2 ) {
            $place = array( intval( count( $taxonomies ) / 3 ) * 3 + 1 );
            $add = " col-md-offset-2 ";
        }
        elseif( ( count( $taxonomies ) % 3 ) == 1 ) {
            $place = array( ( count( $taxonomies ) - 1 ), ( count( $taxonomies ) - 3 ) );
            $add = " col-md-offset-2 ";
        }
        else {
            $place = array( 0 );
		...
		/*if($tax_slug=='pa_garantiya')
			echo "<div class='col-md-2'></div>";*/
        ?>
        <div data-css-class="woof_container_<?php echo $tax_slug ?>" class="<?= (in_array($counter, $place)) ? $add : ''; ?> col-md-4 woof_container woof_container_<?php echo $woof_settings['tax_type'][$tax_slug] ?> woof_container_<?php echo $tax_slug ?> woof_container_<?php echo $counter ?> woof_container_<?php echo $primax_class ?>">
            <div class="woof_container_overlay_item"></div>
			<div class="woof_container_filter_head"><?= $taxonomies_info[$tax_slug]->labels->name; ?></div>
