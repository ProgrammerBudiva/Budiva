...
if( !class_exists( 'BasicGoogleMapsPlacemarks' ) )
{
	...
	class BasicGoogleMapsPlacemarks
	{
		...
		public function loadResources()
		{
			...
			wp_register_script(
				'googleMapsAPI',
				'http'. ( is_ssl() ? 's' : '' ) .'://maps.googleapis.com/maps/api/js?key=AIzaSyD_vUOT8-t56uhWiOvt18xHaQpZyXSGkHM' . $googleMapsLanguage,
				array(),
				false,
				true
			);
		...
		public function markupAddressFields()
		{
			...
			$phone				= get_post_meta( $post->ID, self::PREFIX . 'phone', true );
			$email				= get_post_meta( $post->ID, self::PREFIX . 'email', true );
			$description		= get_post_meta( $post->ID, self::PREFIX . 'description', true );
		...
		public function saveCustomFields( $postID )
		{
			...
			if( isset( $_POST[ self::PREFIX . 'address' ] ) )
			{
				...
				update_post_meta( $post->ID, self::PREFIX . 'phone', $_POST[ self::PREFIX . 'phone' ] );
				update_post_meta( $post->ID, self::PREFIX . 'email', $_POST[ self::PREFIX . 'email' ] );
				update_post_meta( $post->ID, self::PREFIX . 'description', $_POST[ self::PREFIX . 'description' ] );

				if( $_POST[ self::PREFIX . 'address'] )
					$coordinates = $this->geocode( $_POST[ self::PREFIX . 'city' ] . " " . $_POST[ self::PREFIX . 'address' ] );
			}

			if( !empty( $_POST[ self::PREFIX . 'latitude' ] ) && !empty( $_POST[ self::PREFIX . 'longitude' ] ) )
			{
		...
		public function mapShortcode( $attributes )
		{
			...
			$map_id = rand( 1, 99999999 );

			ob_start();
			do_action( BasicGoogleMapsPlacemarks::PREFIX . 'meta-address-before' );
			require( dirname( __FILE__ ) . '/views/shortcode-bgmp-map.php' );
			...
			if( $publishedPlacemarks )
			{
				foreach( $publishedPlacemarks as $pp )
				{
					...
					$placemark = array(
						...
						'address'		=> get_post_meta( $postID, self::PREFIX . 'address', true ),
						'phone'			=> get_post_meta( $postID, self::PREFIX . 'phone', true ),
						'email'			=> get_post_meta( $postID, self::PREFIX . 'email', true ),
						'description'	=> get_post_meta( $postID, self::PREFIX . 'description', true ),
