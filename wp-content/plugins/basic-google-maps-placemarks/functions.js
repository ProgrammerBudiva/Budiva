...
function bgmp_wrapper( $ )
{
	...
	$.bgmp = 
	{
		initAll: function()
		{
			bgmpDataAll.forEach(function(item, i, arr){
				//console.log(item);
				$.bgmp.init(item, i);
			});
		},
		 ...
		 * @author Ian Dunn <ian@iandunn.name>
		 */
		init : function( bgmpData, map_id )
		{
			// Initialize variables
			$.bgmp.prefix				= 'bgmp_';
			$.bgmp.name					= 'Basic Google Maps Placemarks';
			$.bgmp.canvas				= document.getElementById( $.bgmp.prefix + 'map-canvas-' + map_id );		// We have to use getElementById instead of a jQuery selector here in order to pass it to the Maps API.
			$.bgmp.map					= undefined;
			...
			if( bgmpData.markers.length > 0 )
			{
				for( var m in bgmpData.markers )
				{
					var myLatLng = new google.maps.LatLng(
						parseFloat( bgmpData.markers[m][ 'latitude' ].replace( ',', '.' ) ),
						parseFloat( bgmpData.markers[m][ 'longitude' ].replace( ',', '.' ) )
					);
					latlngbounds.extend(myLatLng);
				}
			}
			$.bgmp.map.setCenter( latlngbounds.getCenter(), $.bgmp.map.fitBounds(latlngbounds));
		},
		 ...
		 * @author Ian Dunn <ian@iandunn.name>
		 */
		buildMap : function(bgmpData, map_id)
		{
			var mapOptions;
			...
			$( '#' + $.bgmp.prefix + 'map-canvas-' + map_id ).css( 'width', '100%' );		// @todo use $.bgmp.canvas intead of hardcoding it?
			$( '#' + $.bgmp.prefix + 'map-canvas-' + map_id ).css( 'height', bgmpData.height );
			...
			$.bgmp.addPlacemarks( $.bgmp.map, bgmpData );		// @todo not supposed to add them when clustering is enabled? http://www.youtube.com/watch?v=Z2VF9uKbQjI
		 ...
		 */
		addPlacemarks : function( map, bgmpData )
		{
			...
			if( bgmpData.markers.length > 0 )
			{
				for( var m in bgmpData.markers )
				{
					$.bgmp.createMarker(
						...
						bgmpData.markers[ m ][ 'phone' ],
						bgmpData.markers[ m ][ 'email' ]
					);
				}
			}
		},
		 ...
		 */
		createMarker : function( map, id, title, latitude, longitude, details, icon, zIndex, address, phone, email )
		{
			...
			infoWindowContent = '<div class="gm-infowindow">'
				+ '<p class="gm-address">' + address + '</p>'
				+ '<p class="gm-phone">' + phone + '</p>'
				+ '<p class="gm-email">' + email + '</p>'
				+ '</div>';
			...
			{	
				...
				var image = new google.maps.MarkerImage('/budiva/wp-content/themes/budiva/img/map-label.png',
				...
				{
					$.bgmp.openInfoWindow( map, marker, infoWindowContent, address, phone, email );
				} );
