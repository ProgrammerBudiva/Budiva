...
class Options_Machine
{
        ...
        foreach( $options as $value ) {
            ...
            if( !empty( $smof_data ) || !empty( $data ) ) {
                ...
                switch ( $value['type'] ) {
                    ...
                    case 'slider':
                        $big = ( isset( $value['big'] ) && $value['big'] == true ) ? true : false;
                        $output .= '<div class="slider"><ul id="' . $value['id'] . '" data-big="' . intval( $big ) . '">';
                        $slides = $smof_data[$value['id']];
                        $count = count( $slides );
                        if( $count < 2 ) {
                            $oldorder = 1;
                            $order = 1;
                            $output .= Options_Machine::optionsframework_slider_function( $value['id'], $value['std'], $oldorder, $order, $big );
                        }
                        else {
                            $i = 0;
     ...
     * @return string
     */
    public static function optionsframework_slider_function( $id, $std, $oldorder, $order, $big = false ) {
