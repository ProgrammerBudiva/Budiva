...
/**
 * Display the Featured Image size.
 */
add_action( 'admin_footer', 'admin_display_featured_imafe_size' );
function admin_display_featured_imafe_size() {
	global $pagenow, $typenow;

	if ( ( $pagenow == 'post.php' || $pagenow == 'post-new.php' ) && ( $typenow == 'product' || $typenow == 'product' ) ) {
		$featured_image_size = [
			'w' => 270,
			'h' => 270,
		];

		/*if ( ! $featured_image_size['w'] ) {
			$featured_image_size['w'] = htmlspecialchars( '[full width]' );
		}

		if ( ! $featured_image_size['h'] ) {
			$featured_image_size['h'] = htmlspecialchars( '[any height]' );
		}*/
		?>
		<script type="text/javascript">
			jQuery( document ).ready( function ( $ ) {
				$( '.inside p:eq(0)', '#postimagediv' ).after( '<p class="howto">Размер: <?php echo $featured_image_size['w']; ?>x<?php echo $featured_image_size['h']; ?>px</p>' );
			} );
		</script>
		<?php
	}
